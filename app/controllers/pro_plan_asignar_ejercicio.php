...
try {
    ...
    if (!$usuarioId || !$ejercicioId) throw new Exception('Datos incompletos');

    $profesional = $authController->getCurrentUser();
    $model = new Plan();
    $ok = $model->asignarEjercicio($usuarioId, $ejercicioId, $profesional['correo'], $notas);

    echo json_encode($ok
        ? ['success' => true,  'message' => 'Ejercicio asignado al plan']
